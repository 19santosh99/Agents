...
def handle_tool_calls(messages, ai_response):
    """
    Handles the execution of tool calls requested by the AI and updates the message history.
    
    Args:
        messages (list): The current conversation history
        ai_response (AIMessage): The AI's response containing tool calls
        
    Returns:
        bool: True if tool calls were handled, False otherwise
    """
...
def prompt_ai(messages, nested_calls=0):
    """
    Generates AI responses with streaming support and handles tool execution.
    
    Args:
        messages (list): The conversation history
        nested_calls (int): Counter for recursive tool call depth
        
    Yields:
        str: Streamed chunks of the AI's response
        
    Raises:
        Exception: If tool calling depth exceeds 5 levels
    """
    if nested_calls > 5:
        raise Exception("AI tool calling too")
    ...
    # Stream and accumulate AI response
    response = None
    for chunk in asana_chatbot_with_tools.stream(messages):
        if response is None:
            response = chunk
        else:
    ...
    # Handle any tool calls and generate additional responses if needed
    if handle_tool_calls(messages, response):
        # Generate new response incorporating tool results
        additional_stream = prompt_ai(messages, nested_calls + 1)
        for chunk in additional_stream:
            yield chunk
